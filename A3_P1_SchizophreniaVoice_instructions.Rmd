...
read_pitch <- function(filename) {
    ...
    data <- unlist(regmatches(filename, gregexpr("[[:digit:]]+\\.*[[:digit:]]*", filename)))
...
df <- as.data.frame(t(unlist(regmatches("Study1D0S101T2_f0", gregexpr("[[:digit:]]+\\.*[[:digit:]]*", "Study1D0S101T2_f0")))))

#Renaming the columns 
df <- test_data %>% t() %>% as.data.frame %>% rename(study=V1, diagnosis = V2, subj= V3, trial = V4, f=V5) %>% select(-c(f))
