...
#test_data = read_pitch("Study1D0S101T4_f0.txt")

listed<-list.files(path="C:/Users/stine/OneDrive/Cognitive Science/3rd_Semester/exp_meth_3/git/Assignment-3", pattern="*.txt") %>% 
    purrr::map_df(read_pitch) 


#Renaming the columns 
df <- listed %>% as.data.frame %>% rename(study=V1, diagnosis = V2, subj= V3, trial = V4, f=V5, p=V6) %>% select(-c(f, p))
...
demo <- read.csv("DemographicData.csv", header = T, sep = ";")

demo$Diagnosis <- ifelse(demo$Diagnosis=="Control", 0, 1)

#Editing the name of participant to subj
demo <- demo %>% rename(study=Study, diagnosis = Diagnosis, subj= Participant)

# then duration data
dur <- read.delim("Articulation.txt", header = T, sep = ",")

#Extracting the trail and particpant numbers to their own columns 
df <- regmatches(dur$soundname, gregexpr("[[:digit:]]+\\.*[[:digit:]]*", dur$soundname))

library(stringi)
res <- as.data.frame(t(stri_list2matrix(df)))
res <- res %>% rename(study=V1, diagnosis = V2, subj= V3, trial = V4, f=V5)
dur <- cbind(res, dur)

dur <- dur %>% rename(dur = dur..s., phonationtime=phonationtime..s., speechrate = speechrate..nsyll.dur., articulation= articulation.rate..nsyll...phonationtime., ASD_speakingtime = ASD..speakingtime.nsyll.) %>% select(-c(f, soundname))
...
df <- merge(df, demo, all=T)

#Removing study 6 and 7 as they have many NAs 
...
```{r}
#read df 
df <- read.csv("df.csv", header = T, sep = ",")
#Excluding all recordings without complete data 
df <- df[complete.cases(df$diagnosis),]
df <- df[complete.cases(df$speechrate),]
df <- df[complete.cases(df$ASD_speakingtime),]
df <- df[complete.cases(df$inner_quartil_range),]
df <- df[complete.cases(df$npause),]
df <- df[complete.cases(df$dur),]
...
df %>% group_by(diagnosis, study) %>% summarise(n())

df %>% group_by(diagnosis, study) %>% dplyr::summarise(N = n(), MeanAge = mean(Age, NA.RM=T), SDAge = sd(Age, na.rm = T), FemaleN=sum(Gender=="F"), MeanSANS=mean(SANS, NA.RM=T), MeanSAPS=mean(SAPS, NA.RM=T))
...
```{r}
# we start by finding out what random effects to make 
# library(lme4)
# library(lmerTest)
# library(ggplot2)
...
model2 <- lmer(iqr ~ diagnosis + (1|uID), data=df, REML=F)
summary(model2)

# Checking whether any model is significantly better 
anova(model_iqr, model2)
# We take the first as it is significantly better 

#Making model for speakingtime
model_speakingtime <- lmer(speakingtime ~ diagnosis + (1+diagnosis|uPairID), data=df, REML=F)
summary(model_speakingtime)

# Model for speechrate 
model_speechrate <- lmer(speechrate ~ diagnosis + (1+diagnosis|uPairID), data=df, REML=F)
summary(model_speechrate)

#Making model for pause duration 
model_pause_dur <- lmer(pause_dur ~ diagnosis + (1+diagnosis|uPairID), data=df, REML =F)
summary(model_pause_dur)


# Describe the accoustic features of a schizophrenic voice 
#IQR
ggplot(df, aes(diagnosis, iqr))+ geom_boxplot()
...
anova(model_iqr, model_pause_dur, model_speechrate, model_speakingtime)
